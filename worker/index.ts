...
function readCacheHeaders(isAuthenticated: boolean): Record<string, string> {
  return isAuthenticated ? {} : { 'Cache-Control': 'public, max-age=0, s-maxage=30' };
}
    ...
    if (url.pathname.startsWith('/api/')) {
      ...
      try {
        ...
        if (path === 'auth/enabled' && method === 'GET') {
          ...
          return createJsonResponse({ enabled: authEnabled, guestAvailable }, request);
        }
        ...
        if (path === 'icon' && method === 'GET') {
          ...
          try {
            ...
            const upstream = await fetch(target, {
              ...
              headers: {
                'User-Agent': 'Mozilla/5.0 (compatible; Navihive/1.0; +https://navihive.dev)',
        ...
        if (path === 'groups-with-sites' && method === 'GET') {
          ...
          if (!isAuthenticated) {
            ...
            return createJsonResponse(filteredGroups, request, {
              headers: readCacheHeaders(isAuthenticated),
            });
          }
        ...
        // GET /api/groups 获取所有分组
          ...
          if (
            ...
            (typeof data.is_public !== 'number' || (data.is_public !== 0 && data.is_public !== 1))
          ) {
            return createJsonResponse(
              {
                success: false,
        ...
        // 1. 批量移动站点到其他分组 (PUT sites/batch-move)
          ...
          if (!data.target_group_id) {
            return createJsonResponse({ success: false, message: '请提供目标分组ID' }, request, {
              status: 400,
            });
          }
        ...
        // 书签脚本添加站点 API - 支持跨域
          ...
          try {
            ...
            return createJsonResponse({ success: false, message: '无效的URL格式' }, request, {
              status: 400,
            });
          }
 ...
 * 并发检测链接可用性（限制并发数）
 */
async function checkLinksConcurrent(
  urls: string[],
